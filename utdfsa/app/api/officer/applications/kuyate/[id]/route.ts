// ── route.ts ─────────────────────────────────────────────
// PATCH /api/officer/applications/kuyate/[id] — update kuyate application status (sends a
// notification email) and/or pamilya assignment (members.pamilya, no email)
//
// data:  kuyate_applications, members (pamilya field)
...
//        pamilya assignment is independent of status/email — it's the applicant's real pam
//        (members.pamilya), not their free-text pamilya_name preference used in the email.
...
export async function PATCH(req: Request, { params }: RouteContext) {
  ...
  // ── status update ─────────────────────────────────────────
  if (status !== undefined && appRow.status !== status) {
  ...
  // and is independent of the kuyate's own free-text pamilya_name preference used above
  if (pamilya !== undefined) {
    ...
    if (memberError) {
      console.error('[kuyate/[id]] pamilya update error:', memberError)
