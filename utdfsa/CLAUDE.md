...
- `npm test` — Node's built-in runner (`node --test`, no test framework dependency) over `lib/**/*.test.ts`. 80 tests across 12 files covering the pure business logic that gates money and access: `lib/membership.ts` (`isMembershipActive` status×expiry matrix), `lib/events/pricing.ts` (early-bird window, member/guest tiers), `lib/events/registration-ownership.ts` (`shouldUseMemberPath`), `lib/events/fulfillment.ts` (`classifyFulfillment` fulfill/retry/refund decision), `lib/events/refund-error.ts` (`classifyRefundError` settled/retry/stuck decision for a failed stripe auto-refund), `lib/events/session-expiry.ts` (`resolveSessionExpiry` early-bird min/max clamp), `lib/format.ts` (`splitOAuthName`), `lib/member-type-label.ts` (member-type display labels), `lib/membership-expiry.ts` (`computeMembershipExpiry`, the America/Chicago-anchored renewal cutoff), `lib/reveal-visibility.ts` (`isPastRevealLine`/`revealRootMargin`, the scroll-reveal threshold that keeps a tall above-the-fold card from staying blank), `lib/membership-activation.ts` (`shouldActivateFromSession`, the onboarding fallback's payment/ownership/replay gate for a dropped stripe webhook), `lib/events/scan-retry.ts` (`isRecoveredCheckIn`, the ticket scanner's recovery for a check-in write that committed after its request timed out). Coverage is deliberately narrow — there are no component, route, or integration tests, so a regression in a client component or an API route's control flow is caught by lint + the manual critical-flow checklist, not by CI. Logic extracted for testability should be a pure function in `lib/`, matching the twelve above. For local webhook testing (`stripe listen --forward-to localhost:3000/api/stripe-webhook`), see `.env.example`.
...
3. **`lib/auth.ts`** — in-route/in-page guards: `requireUser()` (session only, caller picks the failure response), `requireOfficer()` (adds role check; `{ verify: 'claims' }` verifies the session token locally instead of calling the auth server — used by the door scanner, see the file header for the ≤1h signed-out-elsewhere tradeoff), `requireActiveMember()` (page-only, redirects itself — defense-in-depth mirror of the middleware gate), `assertActiveMember()` (shared predicate for pages that already fetched their own member row).
...
The webhook's idempotency ledger (`stripe_events`) is a **fulfillment lease**, not a tombstone: a claim row means "done" only once `fulfilled_at` is stamped. A delivery that collides with an existing claim takes it over if that claim is unfulfilled and older than `LEASE_STALE_MS`, instead of no-opping. Nothing is ever rolled back on failure — a 500 deliberately leaves `fulfilled_at` null so Stripe's retry can resume. This replaced a release-the-claim-on-failure design that dropped a paid membership in production on 2026-09-14, when the Supabase Data API 504'd under CPU throttling and the releasing DELETE committed *after* the retry had already collided with the surviving row (Stripe then held a 200 and never retried). `scripts/reconcile-stripe.mjs` (`npm run reconcile:stripe`) sweeps Stripe for payments that were never fulfilled — unfulfilled leases, inactive members holding a session id, and bound pending carts — and repairs memberships with `--fix`.

Paid event tickets use a **pending-cart architecture**, not a pre-payment `event_registrations` row: `app/api/events/register/route.ts` writes the cart to `pending_registrations` (keyed by `stripe_checkout_session_id`, not by identity — see the file's header comment for why), creates the Stripe session, and binds the two together. `app/api/stripe-webhook/route.ts` materializes the real `event_registrations` row + `registration_tickets` only on `checkout.session.completed`, via `lib/events/fulfillment.ts`'s `classifyFulfillment()` (fulfill/retry/refund). `event_registrations` is paid-only for events created under this model — free events still write it directly and immediately as `'paid'` (no payment to wait on, so no pending state). Members are capped at one paid ticket per event (`unique_member_event_registration` + `check_member_single_ticket`, enforcing the membership discount); guests may place multiple paid orders for the same event — that's normal ecommerce behavior, not a bug. A refund that can't be fulfilled (stale session, already-registered conflict) never fails silently: `lib/events/refund-error.ts`'s `classifyRefundError()` distinguishes a transient stripe failure (500s so stripe redelivers and retries the refund) from a permanent one (logged CRITICAL, returns 200 for manual reconciliation — retrying wouldn't fix it). **Transition note:** `stripe-webhook/route.ts` still carries a legacy branch (dispatched on `metadata.registration_id` instead of `metadata.pending_id`) for sessions created before this model shipped; remove it, and purge any leftover non-paid `event_registrations` rows, no sooner than 48h after that deploy.
