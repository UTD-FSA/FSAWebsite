...
// (black) added alongside the original "Become a Member" button.
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 sm:gap-4 z-20 px-6 text-center">
      ...
      <div
        ref={ctaGroupRef}
        style={{ opacity: 0 }}
        className="mt-1 sm:mt-2 pointer-events-auto flex items-center gap-3"
      >
        <Link
          href="/membership"
          className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-accent-green text-[#08130a] font-display font-extrabold text-sm sm:text-base tracking-[0.01em] hover:brightness-[1.08] active:scale-[0.98] transition-all duration-200"
        >
          Become a Member
        </Link>
        <a
          href="#upcoming-events"
          className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-black text-white border border-white/20 font-display font-extrabold text-sm sm:text-base tracking-[0.01em] hover:brightness-125 active:scale-[0.98] transition-all duration-200"
        >
          Upcoming Events
        </a>
      </div>
