...
- `npm test` — Node's built-in runner (`node --test`, no test framework dependency) over `lib/**/*.test.ts`. 38 tests across 7 files covering the pure business logic that gates money and access: `lib/membership.ts` (`isMembershipActive` status×expiry matrix), `lib/events/pricing.ts` (early-bird window, member/guest tiers), `lib/events/registration-ownership.ts` (`shouldUseMemberPath`), `lib/events/fulfillment.ts` (`classifyFulfillment` fulfill/retry/refund decision), `lib/events/refund-error.ts` (`classifyRefundError` settled/retry/stuck decision for a failed stripe auto-refund), `lib/events/session-expiry.ts` (`resolveSessionExpiry` early-bird min/max clamp), `lib/format.ts` (`splitOAuthName`). Coverage is deliberately narrow — there are no component, route, or integration tests, so a regression in a client component or an API route's control flow is caught by lint + the manual critical-flow checklist, not by CI. Logic extracted for testability should be a pure function in `lib/`, matching the seven above. For local webhook testing (`stripe listen --forward-to localhost:3000/api/stripe-webhook`), see `.env.example`.
